...
Мы в качестве пути к директории укажем наш путь к скачанному репозиторию (~/project), а лучше непосредственно к папке src: (~/project/<склонированный_проект>/src). Таким образом мы можем работать с данным репозиторием, используя VsCode хостовой машины!
